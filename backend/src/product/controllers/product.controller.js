...
// console.log(req.query);
    ...
    if (price) {
      ...
      const lte = price.lte;
      // console.log(gte,lte,price);
      if (gte) filters.price = { ...filters.price, $gte: gte };
      if (lte) filters.price = { ...filters.price, $lte: lte };
    }
    if (rating) {
      const gte = rating.gte;
            const lte = rating.lte;
            if (gte) filters.rating = { ...filters.rating, $gte: gte };
            if (lte) filters.rating = { ...filters.rating, $lte: lte };
    const products = await getAllProductsRepo(keyword, page, filters);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    const isReviewExistIndex = reviews.findIndex((rev) => {
      console.log(rev._id.toString() === reviewId.toString() && rev.user.toString()===userId.toString());
      return rev._id.toString() === reviewId.toString() && rev.user.toString()===userId.toString();
    });
    if (isReviewExistIndex < 0) {
      return next(new ErrorHandler(400, "review doesn't exist or you are not authorized to delete someone else's review"));
    }
